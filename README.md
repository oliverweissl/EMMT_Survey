...
The deployment-ready script is also included at `google-apps-script/Code.gs`. It accepts both one response and a `{batch: true, responses: [...]}` payload, then writes one Sheet row per response. After replacing the Apps Script code, deploy a new version. Opening the `/exec` URL in a browser should show `{"ok":true,"service":"bbox-survey"}`; if it does not, the deployment URL or access setting is wrong.
